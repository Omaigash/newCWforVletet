...
void function_list() {
    ...
    puts("9 - Отсортировать предложения по возрастанию количества слов, при равенстве - по длине.\n");
}
...
int compare_sentences_descending(const void* a, const void* b) {
...
int compare_sentences_ascending(const void* a, const void* b) {
    const char* sentence_a = *(const char**)a;
    const char* sentence_b = *(const char**)b;
    int word_count_a = count_words(sentence_a);
    int word_count_b = count_words(sentence_b);
...
void sort_sentences_by_word_count_descending(SentenceArray* array) {
    qsort(array->sentences, array->count, sizeof(char*), compare_sentences_descending);
}

void sort_sentences_by_word_count_ascending(SentenceArray* array) {
...
int main() {
    ...
    case 1:
        ...
        break;
    case 2:
        sort_sentences_by_word_count_descending(&array);
        for (size_t i = 0; i < array.count; i++) {
            printf("%s\n", array.sentences[i]);
        }
        break;
    ...
    case 4:
        ...
        break;
    case 9:
        sort_sentences_by_word_count_ascending(&array);
        for (size_t i = 0; i < array.count; i++) {
            printf("%s\n", array.sentences[i]);
        }
        break;
